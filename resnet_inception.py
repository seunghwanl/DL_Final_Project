...
def resnet_model_fn(features, labels, mode): 
    ...
    with tf.device('/device:GPU:0'):
        ...
        for group_i, group in enumerate(groups):
            ...
            #what I am adding, what comes out must be the same
            try:
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:  
            learning_rate_with_decay = tf.train.exponential_decay(learning_rate=0.045, global_step=tf.train.get_global_step(), decay_steps=2000, decay_rate=0.94, staircase=True)
            optimizer = tf.train.RMSPropOptimizer(learning_rate_with_decay)
            ...
            logging_hook = tf.train.LoggingTensorHook(tensors=tensors_log, every_n_iter=100)
...
classifier = tf.estimator.Estimator(model_fn=resnet_model_fn, model_dir="/Users/snehanagaraj/Documents/DL/Project/WDN/wide_dense_net_v10/cifar", config=tf.estimator.RunConfig())
