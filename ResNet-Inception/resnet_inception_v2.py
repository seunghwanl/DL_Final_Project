...
def resnet_model_fn(features, labels, mode): 
    ...
    with tf.device('/device:GPU:0'):
        ...
        #groups = [
        #    Group(6, 32), Group(12, 64),
        #    Group(24, 64), Group(16, 128)
        #]
        ...
        with tf.variable_scope('conv_layer2'):
            ...
            tf.summary.histogram('weights_conv_group_0_conv_project', net) 
        ...
        for group_i, group in enumerate(groups):
            ...
            net = net + input_net
            tf.summary.histogram('weights_conv_group_%d/_conv_residue' % group_i, net) 
            try:
                next_group = groups[group_i + 1]
                with tf.variable_scope('group_%d/conv_project' % group_i):
                    net = tf.layers.conv2d(
                        net,
                        ...
                        activation=None,
                        bias_initializer=None)
                    tf.summary.histogram('weights_conv_group_%d/_conv_project' % group_i, net) 
            except IndexError:
                pass
